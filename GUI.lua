...
-- =========================
-- == CRIANDO OS BOTÕES E PÁGINAS
-- =========================

local btnInicio, pageInicio, scrollInicio = createMenuButton("Início", 1, MenuFrame, ContentFrame)
local btnInfo, pageInfo, scrollInfo = createMenuButton("Informações", 2, MenuFrame, ContentFrame)
local btnNovo, pageNovo, scrollNovo = createMenuButton("Novo Menu", 3, MenuFrame, ContentFrame)
...
-- Exemplo completo: Dois blocos independentes na mesma aba,
-- cada bloco tem ContentButton e SwitchButton que usam sempre o ÚLTIMO campo alterado.
...
do
    ...
    local frutaState = {
        selecionado = nil,
        digitado = nil,
        ultimo = nil -- "selecionado" ou "digitado"
    }

    createDropdown(1, scrollInicio, "Selecione uma fruta", { "Maçã", "Banana", "Melancia" }, function(valor)
        frutaState.selecionado = valor
        frutaState.ultimo = "selecionado"
    end)

    createInputField(2, scrollInicio, "Ou digite a fruta", "Ex: Uva", function(valor)
        frutaState.digitado = valor
        frutaState.ultimo = "digitado"
    end)

    local function obterFruta()
        if frutaState.ultimo == "selecionado" then
            return frutaState.selecionado
        elseif frutaState.ultimo == "digitado" then
            return frutaState.digitado
        end
        return nil
    end

    createContentButton("Ação da fruta", 3, scrollInicio, function()
        local fruta = obterFruta()
        if fruta and fruta ~= "" then
            print("Ação sobre a fruta:", fruta)
        else
            print("Selecione ou digite uma fruta!")
        end
    end)

    createSwitchButton("Switch da fruta", 4, scrollInicio, function(state)
        local fruta = obterFruta()
        if state and fruta and fruta ~= "" then
            print("Switch ligado! Fruta:", fruta)
        elseif not state then
            print("Switch desligado (fruta)!")
        else
            print("Selecione ou digite uma fruta para usar com o switch!")
        end
    end)
end
...
do
    ...
    createDropdown(5, scrollInicio, "Selecione um animal", { "Porco", "Cachorro", "Gato" }, function(valor)
        animalState.selecionado = valor
        animalState.ultimo = "selecionado"
    end)

    createInputField(6, scrollInicio, "Ou digite o animal", "Ex: Cavalo", function(valor)
        animalState.digitado = valor
        animalState.ultimo = "digitado"
    end)

    local function obterAnimal()
        if animalState.ultimo == "selecionado" then
            return animalState.selecionado
        elseif animalState.ultimo == "digitado" then
            return animalState.digitado
        end
        return nil
    end

    createContentButton("Ação do animal", 7, scrollInicio, function()
        local animal = obterAnimal()
        if animal and animal ~= "" then
            print("Ação sobre o animal:", animal)
        else
            print("Selecione ou digite um animal!")
        end
    end)

    createSwitchButton("Switch do animal", 8, scrollInicio, function(state)
        local animal = obterAnimal()
        if state and animal and animal ~= "" then
            print("Switch ligado! Animal:", animal)
        elseif not state then
            print("Switch desligado (animal)!")
        else
            print("Selecione ou digite um animal para usar com o switch!")
        end
    end)
end

createInfoLabel("Este é um painel de informações.", scrollInfo, 1)

createContentButton("Clique aqui!", 1, scrollNovo, function()
    print("Você clicou no botão da página Novo Menu!")
end)