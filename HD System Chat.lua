...
-- Adição do novo comando de ajuda para dúvidas com timeout de 30 segundos para resposta

elseif cmd == "ajuda" then
    -- Lista de dúvidas
    local duvidas = {
        [1] = "Como uso os comandos?",
        [2] = "Qual o prefixo atual?",
        [3] = "Como virar ADM?",
        [4] = "Como uso o comando /kill?",
        [5] = "Para que serve o comando /tp?",
        [6] = "Como envio uma mensagem personalizada?",
        [7] = "Como ganhar pontos?",
        [8] = "Como vejo meu rank?",
        [9] = "Como vejo a versão do sistema?",
        [10] = "Outros (Falar com ADM/DEV)"
    }
    -- Mensagem da lista de dúvidas
    local lista = "Oi\r[Ajuda]: Escolha o número da sua dúvida:\r"
    for i = 1, 10 do
        lista = lista .. tostring(i) .. ". " .. duvidas[i] .. "\r"
    end
    SendChatMessage(lista)

    -- Controle de tempo e bloqueio para evitar múltiplas conexões
    if player:GetAttribute("AjudaTimeout") then
        SendChatMessage("Oi\r[Ajuda]: Você já está respondendo uma dúvida. Aguarde o tempo acabar para usar novamente!")
        return
    end
    player:SetAttribute("AjudaTimeout", true)

    local ajudaConnection
    local responded = false

    -- Timer de 30 segundos
    task.delay(30, function()
        if not responded then
            SendChatMessage("Oi\r[Ajuda]: Tempo esgotado! Digite /ajuda novamente para tirar sua dúvida.")
            if ajudaConnection then
                ajudaConnection:Disconnect()
            end
            player:SetAttribute("AjudaTimeout", nil)
        end
    end)

    ajudaConnection = TextChatService.MessageReceived:Connect(function(auxMsg)
        if auxMsg.TextSource.UserId ~= player.UserId then return end
        local escolha = tonumber(auxMsg.Text)
        if not escolha or escolha < 1 or escolha > 10 then
            SendChatMessage("Oi\r[Ajuda]: Número inválido! Digite um número de 1 a 10.")
            return
        end
        responded = true
        player:SetAttribute("AjudaTimeout", nil)
        -- Respostas rápidas para as dúvidas
        local respostas = {
            [1] = "Digite /comandos para ver a lista de comandos disponíveis.",
            [2] = "O prefixo atual é: '" .. CommandPrefix .. "'",
            [3] = "Para se tornar ADM basta conseguir 3 pontos.",
            [4] = "Use /kill [NomeDoJogador] para tentar 'abater' outro jogador.",
            [5] = "O comando /tp [Alvo] to [Destino] leva um jogador até outro.",
            [6] = "Use /msg [texto] para enviar uma mensagem personalizada no chat.",
            [7] = "Acerte as perguntas do comando 'quest' para ganhar pontos.",
            [8] = "Use /rank para ver o seu rank atual.",
            [9] = "Use /versao para ver a versão do sistema.",
            [10] = "Sua dúvida será enviada para um ADM/DEV. Aguarde por uma resposta!"
        }
        -- Enviar resposta
        if escolha >= 1 and escolha <= 9 then
            SendChatMessage("Oi\r[Ajuda]: " .. respostas[escolha])
        elseif escolha == 10 then
            SendChatMessage("Oi\r[Ajuda]: Sua dúvida foi enviada para um ADM/DEV! Aguarde resposta no chat.")
            for _, p in pairs(Players:GetPlayers()) do
                if PlayerClasses[p.UserId] == "ADM" or PlayerClasses[p.UserId] == "✨DEV✨" then
                    SendChatMessage("Oi\r[Sistema]: O jogador " .. player.DisplayName .. " pediu ajuda personalizada! Use /responder " .. player.Name .. " [mensagem] para responder.")
                end
            end
            player:SetAttribute("AguardandoAjuda", true)
        end
        ajudaConnection:Disconnect()
    end)

elseif cmd == "responder" and (PlayerClasses[player.UserId] == "ADM" or PlayerClasses[player.UserId] == "✨DEV✨") then
    local args = message:split(" ")
    local alvoName = args[2]
    local resposta = table.concat(args, " ", 3)
    if not alvoName or resposta == "" then
        SendChatMessage("Oi\r[Erro]: Use /responder [NomeDoJogador] [mensagem]")
        return
    end
    local alvoPlayer = nil
    for _, p in pairs(Players:GetPlayers()) do
        if p.Name:lower() == alvoName:lower() or p.DisplayName:lower() == alvoName:lower() then
            alvoPlayer = p
            break
        end
    end
    if not alvoPlayer or not alvoPlayer:GetAttribute("AguardandoAjuda") then
        SendChatMessage("Oi\r[Erro]: Jogador não está aguardando ajuda.")
        return
    end
    SendChatMessage("Oi\r[Ajuda]: " .. alvoPlayer.DisplayName .. ", um ADM/DEV respondeu: " .. resposta)
    alvoPlayer:SetAttribute("AguardandoAjuda", nil)
end
